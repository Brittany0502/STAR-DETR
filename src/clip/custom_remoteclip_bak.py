...
def tokenize_remote(texts, context_length=77):
    """Tokenize text using open_clip tokenizer"""
    if isinstance(texts, str):
        texts = [texts]

    # Use default model name for tokenizer, or you can make this configurable
    model_name = 'ViT-L-14'
...
class PromptLearner(nn.Module):
    def __init__(self, clip_model, classnames, batch_size=None, n_ctx=16, ctx_init=None, ctx_position='end',
        ...
        tokenized_prompts = torch.cat([tokenize_remote(p) for p in prompts]).to(self.device)
        with torch.no_grad():
...
class CLIPCLS_TTA(nn.Module):
    def __init__(self, device, classnames, arch="ViT-L/14", prompt_prefix=None, only_visual=True, momentum_update=False,
        ...
        self.clip_model, _, _ = load_remote_clip(clip_arch, device)
        self.device = device
        ...
        self.n_cls = len(classnames)
        prompts = [self.prompt_prefix + " " + name + "." for name in classnames]
        self.tokenized_prompts = tokenize_remote(prompts).to(self.device)
    ...
    def reset_classnames_and_state(self, classnames, arch):
        ...
        self.classnames = classnames
        self.tokenized_prompts = torch.cat([tokenize_remote(p) for p in prompts]).to(self.device)

        if not self.only_visual:
            clip_m, _, _ = load_remote_clip(clip_arch, self.device)
...
class CLIPCLS_TTA_Multiple(nn.Module):
    def __init__(self, device, classnames, arch=["ViT-B/16", "ViT-L/14"], prompt_prefix=None, **kwargs):
        ...
        for ar in arch:
            clip_arch = arch_mapping.get(ar, ar)
            clip_model, _, _ = load_remote_clip(clip_arch, device)
            ...
            if hasattr(clip_model.visual, 'input_resolution'):
                ...
                # Default resolution for ViT models
                self.resolutions.append(224)

        self.clip_models = nn.ModuleList(clip_models)
        self.n_model = len(self.clip_models)
        self.default_resolutions = kwargs.get("default_resolutions", 224)
