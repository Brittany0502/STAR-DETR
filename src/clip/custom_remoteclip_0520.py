...
# 预训练模型文件路径
PRETRAINED_MODEL_PATH = '/data/caixinyi/TTA/rtdetrv2_pytorch_TTA-自训练-base-prompt/pretrained/RemoteCLIP-ViT-L-14.pt'
MODEL_NAME = 'ViT-L-14'
clip, _, _transform = open_clip.create_model_and_transforms(MODEL_NAME)
ckpt = torch.load(PRETRAINED_MODEL_PATH, map_location="cpu")
clip.load_state_dict(ckpt)

tokenizer_remote_clip = open_clip.get_tokenizer(MODEL_NAME)
_tokenizer = _Tokenizer()


class ClipImageEncoder(nn.Module):
    def __init__(self, device, arch=MODEL_NAME, image_resolution=224, n_class=1000):
        super(ClipImageEncoder, self).__init__()
        self.encoder = clip.visual
        del clip.transformer
        torch.cuda.empty_cache()
        
        self.cls_head = nn.Linear(clip.visual.output_dim, n_class)
...
class PromptLearner(nn.Module):
    def __init__(self, clip, classnames, batch_size=None, n_ctx=16, ctx_init=None, ctx_position='end', learned_cls=False):
        super().__init__()
        n_cls = len(classnames)
        self.learned_cls = learned_cls
        dtype = clip.visual.conv1.weight.dtype
        self.dtype = dtype
        self.device = clip.visual.conv1.weight.device
        ctx_dim = clip.ln_final.weight.shape[0]
        self.ctx_dim = ctx_dim
        self.batch_size = batch_size

        # self.ctx, prompt_prefix = self.reset_prompt(ctx_dim, ctx_init, clip)
        ...
        if ctx_init:
            ...
            prompt = tokenizer_remote_clip(ctx_init).to(self.device)
            with torch.no_grad():
                embedding = clip.token_embedding(prompt).type(dtype)
        ...
        if not self.learned_cls:
            ...
            name_lens = [len(tokenizer_remote_clip.encode(name)) for name in classnames]
        ...
        tokenized_prompts = torch.cat([tokenizer_remote_clip(p) for p in prompts]).to(self.device)
        with torch.no_grad():
    ...
    def reset_classnames(self, classnames, arch):
        ...
        clip, _, _ = open_clip.create_model_and_transforms(MODEL_NAME)
...
class CLIPCLS_TTA(nn.Module):
    def __init__(self, device, classnames, arch="ViT-L/14", prompt_prefix=None, only_visual=True, momentum_update=False,
        ...
        self.n_cls = len(classnames)
        prompts = [self.prompt_prefix + " " + name + "." for name in classnames]
        self.tokenized_prompts = tokenizer_remote_clip(prompts).to(self.device)
        ...
        with torch.no_grad():
            self.clip_state_dict = copy.deepcopy(self.clip.visual.state_dict())
            self.initial_state_dict = copy.deepcopy(self.clip.visual.state_dict())
            if self.momentum_update:
                self.momentum_state_dict = copy.deepcopy(self.clip.visual.state_dict())
    ...
    def get_class_features(self, tokenized_prompts):
        class_features = self.clip.encode_text(tokenized_prompts)
    ...
    def freeze_parameters(self):
        if self.only_visual:
            for n, p in self.clip.named_parameters():
                ...
                #         p.requires_grad_(False)
        self.clip.transformer.eval()
        self.clip.ln_final.eval()
    ...
    def reset_classnames_and_state(self, classnames, arch):
        ...
        if not self.only_visual:
            clip_m, _, _ = open_clip.create_model_and_transforms(MODEL_NAME)
        ...
        self.clip.visual.load_state_dict(self.clip_state_dict)
        self.initial_state_dict = copy.deepcopy(self.clip.visual.state_dict())
        if self.momentum_update:
            self.momentum_state_dict = copy.deepcopy(self.clip.visual.state_dict())
    ...
    @torch.no_grad()
    def reset(self):
        self.clip.visual.load_state_dict(self.initial_state_dict)
    ...
    def momentum_update_model(self):
        ...
        if self.momentum_update:
            ...
            state_dict = self.clip.visual.state_dict()
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        if self.only_norm:
            for n, m in self.clip.visual.named_modules():
...
class CLIPCLS_TTA_Multiple(nn.Module):
    def __init__(self, device, classnames, arch=["ViT-B/16", "ViT-L/14"], prompt_prefix=None, **kwargs):
        ...
        self.resolutions = []
        for ar in arch:
            clip, _, _ = open_clip.create_model_and_transforms(ar)
            clips.append(clip)
            self.resolutions.append(clip.visual.input_resolution)
        self.clips = nn.ModuleList(clips)
        self.n_model = len(self.clips)
    ...
    def get_class_features(self, tokenized_prompts):
        ...
        for i in range(self.n_model):
            text_feat = self.clips[i].encode_text(tokenized_prompts).float()
            # normalized features
    ...
    def extract_image_features(self, images):
        ...
        for i in range(self.n_model):
            if self.resolutions[i] != self.default_resolutions:
                ...
                tmp_images = images
            image_feat = self.clips[i].encode_image(tmp_images).float()
    ...
    def forward(self, image):
        ...
        for i in range(self.n_model):
            logit_scale = self.clips[i].logit_scale.exp()
