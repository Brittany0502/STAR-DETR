...
### PUT YOUR PATH HERE
DOWNLOAD_ROOT_v2 = '/data/caixinyi/TTA/rtdetrv2_pytorch_TTA-自训练-base-prompt/pretrained/RemoteCLIP-ViT-L-14.pt'
...
class ClipImageEncoder(nn.Module):
    def __init__(self, device, arch="ViT-L/14", image_resolution=224, n_class=1000):
        ...
        clip, _, _transform = open_clip.create_model_and_transforms(DOWNLOAD_ROOT_v2)
...
class CLIPCLS_TTA_Multiple(nn.Module):
    def __init__(self, device, classnames, arch=["ViT-B/16", "ViT-L/14"], prompt_prefix=None, **kwargs):
        ...
        self.resolutions = []
        for ar in arch:
            clip, _, _ = load(ar, device=device, download_root=DOWNLOAD_ROOT)
            clips.append(clip)
