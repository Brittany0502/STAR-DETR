...
__all__ = ['RTDETRTransformerv2','LoRALayer','LoRALinear']
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_ffn(self, tgt):
        if self.enable_lora:
            # (1) 通过第一层 LoRA 包装层（内部会调用 frozen 的 self.linear1）
            x = self.linear1_lora(tgt, self.linear1)
            # (2) 激活 + dropout
            x = self.activation(x)
            x = self.dropout3(x)
            # (3) 通过第二层 LoRA 包装层（内部会调用 frozen 的 self.linear2）
            x = self.linear2_lora(x, self.linear2)
            return x
        else:
            # 直接走原始的两层 FC
            x = self.linear1(tgt)
...
class RTDETRTransformerv2(nn.Module):
    ...
    def __init__(self,
        ...
        # --------lora_idx-----------
        if lora_idx is None:
            # 如果没有指定，且enable_lora为True，默认对所有层使用LoRA
            self.lora_layers = list(range(num_layers)) if enable_lora else []
        else:
            # 如果指定了lora_layers，将其转换为list
            if isinstance(lora_idx, int):
                self.lora_layers = [lora_idx]
            elif isinstance(lora_idx, (list, tuple)):
                self.lora_layers = list(lora_idx)
            else:
                raise ValueError(f"lora_layers must be int, list or tuple, got {type(lora_idx)}")
        ...
        #     enable_lora=self.enable_lora,lora_rank=self.lora_rank,lora_alpha=self.lora_alpha,lora_dropout=self.lora_dropout)
        # self.decoder = TransformerDecoder(hidden_dim, decoder_layer, num_layers, eval_idx)

        decoder_layers = []
        for i in range(num_layers):
            # 判断当前层是否需要启用LoRA
            layer_enable_lora = self.enable_lora and (i in self.lora_layers)
            ...
            decoder_layer = TransformerDecoderLayer(
                ...
                enable_lora=layer_enable_lora,  # 根据层索引决定是否启用LoRA
